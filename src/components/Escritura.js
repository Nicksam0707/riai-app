...
export default function Escritura() {
  ...
  const handleEnviar = async () => {
    ...
    try {
      ...
      const filename = isZip ? "escrituras.zip" : "escritura-processada.pdf";
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a"); a.href = url; a.download = filename;
      document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
    } catch (err) { console.error(err); alert("Falha ao analisar escritura."); }
    finally { setLoading(false); }
  };
  return (
    <div style={{ padding: "1rem", marginTop: "2rem", borderRadius: "12px", boxShadow: "0 2px 8px #0001", background: "#fff", position: "relative" }}>
      <div style={{ position: "absolute", top: 12, left: 20, fontWeight: "bold", fontSize: 20, color: "#2c3e50" }}>Escritura</div>
      <div style={{ height: 32 }} />
      <input type="file" accept=".pdf,.tif,.tiff" multiple onChange={handleChange} style={{ marginBottom: 16 }} />
      {loading ? <SpinnerLogo /> : <button onClick={handleEnviar} disabled={!files.length} style={{ fontWeight: "bold", background: "#2c3e50", color: "#fff", borderRadius: 6, padding: "8px 20px", border: "none" }}>Analisar Escritura</button>}
    </div>
  );
}
